...
#        self.build_defences(game_state)
    ...
    def build_base(self, game_state):
        firewall_locations = [[ 3, 12],[ 24, 12],[ 7, 10],[ 20, 10],[ 11, 8],[ 16, 8]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)
        
        firewall_locations = [[ 0, 13],[ 1, 13],[ 2, 13],[ 3, 13],[ 24, 13],[ 25, 13],[ 26, 13],[ 27, 13]]
        for location in firewall_locations:
            if game_state.can_spawn(FILTER, location):
                game_state.attempt_spawn(FILTER, location)
        
        firewall_locations = [[ 4, 12],[ 23, 12],[ 20, 10],[ 10, 8],[ 12, 8],[ 15, 8],[ 17, 8]]
        for location in firewall_locations:
            if game_state.can_spawn(FILTER, location):
                game_state.attempt_spawn(FILTER, location)
                       
        firewall_locations = [[ 8, 13],[ 9, 13],[ 10, 13],[ 11, 13],[ 16, 13],[ 17, 13],[ 18, 13],[ 19, 13]]
        for location in firewall_locations:
            if game_state.can_spawn(FILTER, location):
                game_state.attempt_spawn(FILTER, location)

        firewall_locations = [[ 4, 13],[ 5, 13],[ 6, 13],[ 7, 13],[ 12, 13],[ 15, 13],[ 20, 13],[ 21, 13],[ 22, 13],[ 23, 13]]
        for location in firewall_locations:
            if game_state.can_spawn(FILTER, location):
                game_state.attempt_spawn(FILTER, location)

        firewall_locations = [[ 1, 12],[ 5, 12],[ 12, 12],[ 15, 12],[ 22, 12],[ 26, 12]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)

                firewall_locations = [[ 12, 11],[ 15, 11]]
        for location in firewall_locations:
            if game_state.can_spawn(ENCRYPTOR, location):
                game_state.attempt_spawn(ENCRYPTOR, location)        

        firewall_locations = [[ 2, 12],[ 6, 12],[ 8, 12],[ 10, 12],[ 17, 12],[ 19, 12],[ 21, 7],[ 25, 12]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)

                firewall_locations = [[ 5, 10],[ 7, 10],[ 13, 10],[ 14, 10],[ 20, 10],[ 22, 10]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)

        firewall_locations = [[ 6, 10],[ 8, 10],[ 19, 10],[ 21, 10]]
        for location in firewall_locations:
            if game_state.can_spawn(DESTRUCTOR, location):
                game_state.attempt_spawn(DESTRUCTOR, location)
    ...
    def deploy_attackers(self, game_state):
        ...
        """
        if game_state.can_spawn(EMP, [13, 0], 3):
            game_state.attempt_spawn(EMP, [13, 0], 3)
 
        """
        Now lets send out 3 Pings to hopefully score, we can spawn multiple 
        information units in the same location.
        """
        if game_state.can_spawn(SCRAMBLER, [14, 0], 3):
            game_state.attempt_spawn(SCRAMBLER, [14,0], 3)
